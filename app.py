...
> 名詞：**圖層**＝一張投影片裡的元素（文字 / 圖片 / 影片 / 形狀）；**文字圖層**只指文字。
> **段（run）**＝同一文字圖層裡「同一種樣式」的連續片段。**段落（group）**＝投影片的分段（主歌/副歌…）。

### ▍套用樣式（最上方）

- **功能**：把參考排版（字體／字級／顏色／位置／對齊／陰影）套到每一張，文字保留。
- **規則邊界**：逐層對應；某張文字層數比樣式多時，多出的層維持原樣。
- **例**：選「單句中英」→ 每張第 1、2 個文字層換成中、英樣式，原文字不變。

---

### 轉換

**繁簡轉換** — 全文繁↔簡（opencc，就地換字、保留樣式與控制字）。

- **規則**：預設繁→簡；勾「反向」則簡→繁。
- **例**：「讚美」→「赞美」（反向則相反）。

**拼音標註** — 每張「第一個文字層的中文」轉拼音（無聲調）覆蓋「第二個文字層」。

- **規則**：需要 ≥2 個文字層；引擎＝opencc(繁→簡)+pypinyin。選項「mi轉換」(預設開)把敬語「祢」的拼音 mi 改成 ni。第一層中文含換行時會先問你拼音要不要跟著斷行。
- **例**：第一層「讚美主」→ 第二層變「zan mei zhu」。

---

### 操作

**反轉圖層順序** — 把每張的圖層順序前後鏡像顛倒。

- **規則邊界**：圖層數 < 2 的投影片不動。
- **例**：圖層 [中, 英, 背景] → [背景, 英, 中]。

**依換行拆分圖層** — 把一個文字圖層裡的每一行各自拆成獨立圖層。

- **規則邊界**：第 0 行留原圖層，其餘各行複製成新圖層接在後面、位置依序略往下；只有一行的圖層不動。
- **例**：一個圖層「主啊\\n我敬拜祢」→ 兩個圖層「主啊」「我敬拜祢」（第二個略低）。

**依圖層拆分投影片** — 把一張裡的多個文字圖層，各自拆成獨立投影片。

- **規則邊界**：每張保留背景等非文字層 ＋ 一個文字層，且該文字層改成全幅 1920×1080 置中；只有 1 個文字層的投影片不動。
- **例**：一張含 [中文層, 英文層] → 兩張，各一層、滿版置中。

**全部投影片加圖層** — 每張投影片各加一個文字圖層。

- **規則邊界**：以檔內既有的文字圖層為範本複製、預設文字「-」；整份找不到可當範本的文字圖層則不動。
- **例**：每張底部多一個「-」文字層，之後可在撰寫頁改。

**合併圖層段落** — 同一文字圖層內因樣式不同被切開的段，合併成單一樣式。

- **規則邊界**：採「首段樣式」（第一段的字體/字級/顏色）重寫整層；**移除「換樣式時多出來的換行」**——樣式邊界上的連續換行數扣一個（1→接成同行、2→留一次），同樣式之間的換行全部保留。只有單段(或更少)的圖層不動。
- **例**：「紅」(紅色)＋「\\n藍」(藍色) → 「紅藍」（同一行）；同樣式「第一行\\n第二行」→ 不變。

**合併雙排** — **段落(group)內**把投影片兩兩合併成上下雙排（前張在上行、後張在下行）。
...
**清除空圖層** — 刪掉內容為空、或還停在預設字「Double-click to edit」的文字圖層。

- **規則邊界**：只動文字圖層；圖片/影片/形狀/背景層不碰。
- **例**：一張 [空文字層, 背景圖] → 只剩 [背景圖]。

**整理空格** — 每行頭尾空白清乾淨、字中間連續空白（半/全形、tab、NBSP、零寬…）併成一個半形空格；換行與空行保留。

- **規則邊界**：選項「連單換行都刪」＝把「單換行」（前後兩行都有字）換成空格接成一行，空行（段落分隔）保留。
- **例**：「　主啊　 \\n敬拜 」→「主啊\\n敬拜」；開「連單換行」時「主\\n愛我」→「主 愛我」，但「第一段\\n\\n第二段」不變。

**清除無圖層投影片** — 刪掉完全沒有圖層的空白投影片。

- **規則邊界**：選項「保留只有背景影片/圖片的投影片」(預設開)＝有背景元素的空頁會留著。
- **例**：純空白頁刪掉；只有背景影片的頁（預設）保留。

**只保留前幾個圖層** — 每張只留前 N 個圖層，其餘刪掉（N 可輸入，預設 2）。

- **規則邊界**：圖層數 ≤ N 的投影片不動。
- **例**：N=2，一張 [中, 英, 拼音, 背景] → [中, 英]。

**刪除每張最後一個圖層** — 把每張投影片最後一個圖層刪掉（不限文字層）。
...
- **功能**：貼上和頁數相同行數的文字，一行對一頁、按文件順序填入指定的文字圖層。
- **規則邊界**：可填頁數＝有文字圖層的投影片數，行數需與其相符才能套用；「覆蓋第幾個文字圖層」填 1～N＝覆蓋每頁第 N 個文字層內容、填 N+1（超過現有層數）＝在每頁最後新增一個文字層放這行；以首字樣式寫入。
- **例**：3 頁、各有 1 個文字層，貼「第一頁/第二頁/第三頁」三行、層數填 1 → 三頁主文被替換；層數填 2 → 每頁尾端各新增一層放這些字。

---

## 其他

- **撰寫頁段落類型「標題」**：套用時自動把首頁首圖層的內文左右加上書名號《》（已包則不重複）。
- **創造的預設檔名**：取首行文字並左右加《》（已包則不重複），此檔名同時是匯出的預設檔名。
- **多樣式換行**：解析頁逐段呈現各種樣式；撰寫頁顯示整層時已移除「換樣式多出來的換行」，雙語單行不再被穿插斷開（同「合併圖層段落」規則）。
"""
