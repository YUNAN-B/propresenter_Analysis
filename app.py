...
支援 ProPresenter 6 的 **.pro6**；ProPresenter 7 的 **.pro** 上傳後會自動轉成 .pro6。

---

## 🚀 三步驟上手

1. **上傳**：把檔案拖進頁面任何地方，或點上傳框（吃 .pro6 / .pro / .xml）。
...
| 我想要⋯⋯ | 去這裡 |
| :-- | :-- |
| 看這份檔有幾張、用什麼字體字級顏色 | **解析** 分頁 |
| 改歌詞的字 | **撰寫** 分頁（改完點別處自動存檔） |
| 標主歌/副歌（分段上色）、設熱鍵、刪投影片 | **撰寫** → 開啟「詳細編輯」 |
| 繁體 ↔ 簡體 | **模板** → 繁簡轉換 |
| 幫中文加拼音 | **模板** → 拼音標註 |
...
### 解析（只看、不會改到檔案）
把整份檔攤開成文字：每張投影片屬於哪個段落、每個圖層的位置/字體/字級/顏色/陰影，
以及完整歌詞。動手改之前，先來這裡確認檔案長什麼樣子。
...
另外：最上方「**樣式**」＝把預設排版（字體/字級/顏色/位置）套到每一張、文字保留；
最下方「**大量填入文字**」＝貼上與頁數相同行數的文字，一行填進一張。
...
### 創造（從純文字生出新檔）
貼歌詞 → 直接產生新 .pro6。預設「空行＝換頁、換行＝分圖層」，可在下拉選單調整。
...
- **投影片（slide）**：一張畫面。
- **圖層（layer）**：一張投影片裡的一塊內容——例如中文一層、英文一層。
- **段落（group）**：投影片的分段（主歌/副歌/橋段⋯），在 ProPresenter 會顯示顏色。
- **熱鍵（hotKey）**：在 ProPresenter 按一個鍵直接跳到那張投影片。
- **段（run）**：同一圖層裡「同一種樣式」的連續文字；撰寫頁以整層為單位編輯。
